...
//공백을 기준으로 문자열 분리 및 하이라이트
function str_highlight($q, $str){
	$q_a = explode(" ", urldecode($q));
	for($i=0; $i<count($q_a); $i++) {
		$str = str_replace($q_a[$i], '<span class="tc_2">'.$q_a[$i].'</span>', $str);
	}
	return $str;
}

//이미지 여부 확인시 사용 - 외부이미지 포함
 function checkRemoteImage($url) {
	$img = getimagesize($url);
	if($img[0] > 0) return true;
	else return false;
}
...
//미리보기시 html decode - encode decode 잘못씀ㅠ
...
function uploadMulti($filearray, $i, $targetdir, $max_size = 10, $allowext = 'image') {
	...
	if($filearray['size'][$i] > $max_size){
		...
		if (!in_array(strtolower($file_ext), $allowext)) return 'error|허용되지 않는 파일타입니다.';
		//if (in_array(strtolower($file_ext), $notAllowExt)) return 'error|허용되지 않는 파일타입니다.';

		//$temp_file_name = str_replace(" ","_",$filearray['name'][$i]);
...
//러닝타임 및 수강시간을 시분초로 출력
function displayTime($w){  
	if($w > 0){
		if($w < 60){
			$dtime = "00:".sprintf('%02d',$w);
		}else if($w >= 3600){
			$h = $w / 3600;
			$m = ($w % 3600) / 60; 
			$s = $w % 60;
			$dtime = sprintf('%02d',$h).":".sprintf('%02d',$m).":".sprintf('%02d',$s);	
		}else{
			$m = $w / 60;
			$s = $w % 60;
			$dtime =sprintf('%02d',$m).":".sprintf('%02d',$s);	
		}
	}else{
		$dtime = "";
	}
...
function getImageInContent($str){
