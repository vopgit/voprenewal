...
function act_popload(iframe) {
    $(iframe).on('load',function(){
        ...
        $(iframe).contents().find('.pop_close,.popbg').click(function() {
            $(iframe).stop().fadeOut(300, function() {
                $(this).closest('.ifr_prWrap').hide();
...
function eve() {
    $.each(['fadeIn', 'hide','fadeOut'], function (i, ev) {
    var el = $.fn[ev];
    $.fn[ev] = function () {
    this.trigger(ev);
...
function iframDisplay(){
    $('#popup_iframe').on('fadeIn',function(){
        $('.ifr_prWrap').fadeIn(300);
    })
    $('#popup_iframe').on('hide',function(){
        $(this).closest('.ifr_prWrap').hide();
    })
}
