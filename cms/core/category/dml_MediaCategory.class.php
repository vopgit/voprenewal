...
if( $_base['method'] == 'POST' && $mode == "insert"){
	...
	if($adm_no == "" || $media_category_name == "") msg_err("주요 등록 항목이 누락되었습니다.");
	...
	$result = $_contents->addMediaCategory($add_data);

	//msg_err("등록에 실패하였습니다bbbb".$result);


	if($result) msg_replace("등록되었습니다", 'list_media_category');
...
if($_base['method'] == 'POST' && $mode == "modify"){
	...
	$result = $_contents->updateMediaCategory($add_data, $where);

	if($result) msg_replace("수정되었습니다", 'list_media_category'.$param['query']);
...
if($_base['method'] == 'POST' && $mode == "selDelete"){
	...
	if($db->query("update TBL_MEDIA set del_tf = 'Y', del_adm = '".$adm_no."', del_date = now(), del_ip = '".$_SERVER["REMOTE_ADDR"]."' where id in (".implode(',', $chk).")")) msg_replace("삭제되었습니다", 'list_media_category'.$param['query']);
...
if($_base['method'] == 'POST' && $mode == "Delete"){
	...
	if($result) msg_replace("삭제되었습니다", 'list_media_category'.$param['query']);
	else msg_err("삭제에 실패하였습니다");
