        ...
        <div class="popup_ty1 popup_con">
            ...
            <div class="popup_contents">
				...
				<div class="board_view type_a">
					...
					<div class="bv_box ">
						<div class="bv_top">
							<p class="bv_tit">[<?=$rs['category']?>] <?=$rs['title']?></p>

							<div class="bv_info_box">
								<ul class="bv_info">
									<li><?=$rs['reg_date']?></li>
									<!--<li>00코멘트</li>-->
									<li data-tit="작성자"><?=$rs['writer_print']?></li>
								</ul>
							</div>
						</div>
							...
							<div class="bv_file_wrap">
								...
								<div class="bv_file_box">
									<?php foreach($rs['file'] as $rdata) { ?>
										<a href="<?=_DOWNLOAD_URL?>?<?=encrypt(time()."||".$rdata['filepath']."||".$rdata['filename'])?>" target="_blank" class="file" title="다운로드"><?=$rdata['filename']?> (<?=formatFileSize($rdata['size'])?>)</a>
									<?php } ?>
									
									<?php if(count($rs['file']) < 1){ ?>
										<p class="ptb_10">등록된 첨부파일이 없습니다.</p>
									<?php } ?>
								</div>
							</div>
