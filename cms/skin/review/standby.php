		...
		if(dup_submit_flag == 'N'){
			...
			if(confirm(_msg)){
				...
				$.ajax({
					type: 'post',
					url:"/review/dml",
					data: {mode: 'DEL_SEL_ID', review_id:review_id, from_page:'standby'},
					success: function(result){
						console.log(result.code);
						if(result.code == "success"){
							dup_submit_flag = 'Y';
							alert("삭제 되었습니다.");
							location.replace("/review/standby<?=$param['query']?>");
						}else{
							dup_submit_flag = 'N';
							alert(result.msg);
						}
		...
		if(dup_submit_flag == 'N'){
			...
			if(confirm(_msg)){
				...
				$.ajax({
					type: 'post',
					url:"/review/dml",
					data: {mode: 'UPDATE_STATUS_SEL_ID', status:param, from_page:'standby'},
					success: function(result){
						console.log(result.code);
...
<div class="contain">
    ...
    <div class="wrap">
        ...
        <form name="frm" id="frm" method="post">
			...
			<input type="hidden" name="from_page" value="standby">
            ...
            <div class="mtb_30 btn_box flex">
                <!--<button type="button" class="btn_lg btn_c2 mr_10"> 선택 편집 완료</button>-->
